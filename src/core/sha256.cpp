...
constexpr std::array<std::uint32_t, 8> kInitialHashes = {
...
void process_block(const std::uint8_t block[64], std::array<std::uint32_t, 8>& hash) {
...
std::string hex_digest(const std::vector<std::uint8_t>& data) {
  std::array<std::uint32_t, 8> hash = kInitialHashes;
  const std::uint64_t bit_len = data.size() * 8;
  ...
  for (std::size_t offset = 0; offset < msg.size(); offset += 64) {
    process_block(msg.data() + offset, hash);
  }
